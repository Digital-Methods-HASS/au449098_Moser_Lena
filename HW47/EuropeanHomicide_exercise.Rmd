...
ggplot(data = Western_Europe, aes(x = Year, y = homicides_per_100k, color = Entity))+
  geom_line()+
  labs(title = "Longterm trends in Homocide", 
...
```{r Longterm Trends in Homicide, eval = TRUE}
ggplot(data = Western_Europe, aes(x = Year, y = homicides_per_100k, color = Entity))+
  geom_line()+
...
```{r Reign among Danish rulers, eval = TRUE}
kings<-read_csv2("data/kongerække3.csv")

kings %>% 
  select(monarch,end_reign,start_reign) %>% 
  ...
  geom_smooth()+
  labs(title = "Pattern of reign duration among Danish rulers", 
       x = "Middle year", 
       y = "Duration of reign")

#Duration of reign seems to rise in the late middle ages, suggesting societal changes that enable rulers to remain for large parts of their lives without being removed, abdicate, or dying soon after being instated as king. 
...
```{r Trends in Homocide per Country, eval = TRUE}
ggplot(data = Western_Europe, aes(x = Year, y = homicides_per_100k, color = Entity))+
  geom_line()+
    facet_wrap(~Entity, ncol = 2,3)+
...
Based on the visualisations it seems that European societies have become less violent over time. The visualisations showing changes in the duration of reign suggests that societal changes happen in the late middle ages that result in more stable periods of reign. Equating stable reign directly to a peaceful society can be a rather fraught approach, as consistent state control doesn't equal a nonviolent society. However, the long-term trends in homicide do seem to show that unlawful killings in European societies have decreased considerably over the last six or seven centuries, at least in civilian society.